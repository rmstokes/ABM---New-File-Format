...
public class Preprocessor {
	...
	public void cdf(ArrayList al, double binSize, String filename,
		...
		//System.out.println("inside cdf -- filename=> "+filename);
		//System.exit(0);
		PrintWriter out;
	...
	public static void main(String[] args) {
		...
		if (args.length > 5) {
			...
			numSubjects = Integer.parseInt(args[6]);
			numTemplates = Integer.parseInt(args[7]);
		  ...
		  for (int subjectNumber = 0; subjectNumber < numSubjects; subjectNumber++) { //
		 ...
		 // **** partial path will need to change to reflect location of files on given machine **** //
			  
		  String partialPath = "C:\\Users\\Robert\\Dropbox\\Bank of America\\Mouse_Movement_Captured_log_Reformatted\\Mouse_Movement_Captured_log\\User"+(subjectNumber+1);
		  
		  // create a log reader object and read in a casis log file to test.
		  
		  // System.out.println("creating log reader."); 
		  numInstances = numTemplates;
		  LogReader lr = new LogReader(partialPath, numInstances); 
		  ArrayList<ArrayList> instances = lr.getUserInstances(); 
		  subjectName = lr.getUserID();
		 ...
		 // ##############################
		  ...
		  preprocessor = new Preprocessor(al); 
		  //System.out.println("starting preprocessor.");
		  preprocessor.start(); // //
		...
		Subject[] subjectList = new Subject[numSubjects];

		// ::start looping::
		for (int subjectNumber = 0; subjectNumber < numSubjects; subjectNumber++) {
			for (int instanceNumber = 0; instanceNumber < numInstances; instanceNumber++) {

				String filepath_toSubjectInstances = "C:\\Users\\Robert\\workspace\\Biometric Engine\\";
				// ex. locate Subject 0 Instance 0
				String fileName = filepath_toSubjectInstances+"Subject " + (subjectNumber) + " Instance "
						+ instanceNumber;
				...
				/*
				 * comment out this section because I'm testing dynamic feature set now.
				...
				*/
				...
				template = new Template(extractedFeatures, (subjectNumber+1),
				...
				*/
				...
				//System.exit(0);
				// each subject should have same# of templates
				...
				for (int i = 0; i < numSubjects; i++) {
					if (subjectList[i] == null && i == subjectNumber) {
						...
						subjectList[i].addSubjectNumber(i+1);
						System.out
	...
	public static double[] calculateCMC(Subject[] subjectList) {
		...
		for (int i = 0; i < subjectList.length; i++) {
			for (int j = 0; j < numTemplates; j++) {
				total++;
				//System.out.println("subjectList[i].getTemplate().get(j).getRank(): "+subjectList[i].getTemplate().get(j).getRank());
				rankCount[subjectList[i].getTemplate().get(j).getRank()-1]++;
				
			}
		}
		
		 System.out.println("end of cmc...");
		 try {
			Thread.sleep(3000);
	...
	public static void calculateROC(Subject[] subjectList,
		...
		System.out.println("inside calculateROC...");
		//System.exit(0);
		...
		while (true) {
			...
			for (int i = 0; i < numSubjects; i++) {
				for (int j = 0; j < numTemplates; j++) {
					...
					for (int k = 0; k < numSubjects; k++) {
						...
						s = subjectList[k];
						//System.out.println("****calling compareTemplateToSubject()*******");
						accepted = compareTemplateToSubject(template, s, threshold);
			...
			//System.exit(0);
			//System.out.println("total: "+total);
			double tar = (double) validAcceptance/18; //18 comes from 6 users * 3 templates a piece
			double far = (double) falseAcceptance/total; 
			double frr = 1-tar;
	...
	public static boolean compareTemplateToSubject(Template t, Subject subject,
			double threshold) {
		//System.out.println("inside compareTemplateToSubject()");
		...
		for (int i = 0; i < numTemplates; i++) {
			distance += t.computeManhattanDistance(subject.getTemplate().get(i));
			//System.out.println("i: "+i+" numTemplates: "+numTemplates);
		}

		double averageDistance = (double) distance / numTemplates;
		
		
		//System.out.println("back in comparedTemplateToSubject() to return...");
		//System.exit(0);
